...
```{r recall_by_make, eval = NOT_CRAN}
...
```{r recall_by_years, eval = NOT_CRAN}
...
```{r count_recall_by_make, eval = NOT_CRAN}
...
```{r recall_details, eval = NOT_CRAN}
recall_windstar <- recall_details(1997118)
t(recall_windstar) # transpose for readability
```
...
```{r plot, eval = NOT_CRAN, fig.width=7, fig.height=5, fig.align='center', warning=FALSE, message=FALSE}
nb.cols <- 11
...
```{r plot2, eval = NOT_CRAN, fig.width=7, fig.height=5, fig.align='center', warnings = FALSE, message = FALSE}
recall_comparison %>%
  ggplot() +
